...
(* :Discussion:
  ...
  -------------------
    ...
    3. If the number of variables (attributes) is k for each i, 1\[LessEqual]i\[LessEqual]k :
...
(*Package Header*)
BeginPackage["AntonAntonov`VariableImportanceByClassifiers`"];
...
Clear[AccuracyByVariableShuffling];

AccuracyByVariableShuffling::noargs = "The first argument is expected to be a classifier. " <>
    "The second argument is expected to be classifier test data. " <>
        "The third optional argument is expected to be a string, a list of strings, or Automatic.";
...
ClassifierQ[ cl_ ] :=
    MatchQ[ cl, _ClassifierFunction] ||
        If[Length[DownValues[EnsembleClassifierMeasurements]] > 0,
...
AccuracyByVariableShuffling[clFunc_?ClassifierQ, testData_?ClassifierDataQ, opts : OptionsPattern[] ] :=
    AccuracyByVariableShuffling[clFunc, testData, Automatic, opts];

AccuracyByVariableShuffling[clFunc_?ClassifierQ, testData_?ClassifierDataQ, variableName_String, opts : OptionsPattern[] ] :=
    AccuracyByVariableShuffling[clFunc, testData, {variableName}, opts];

AccuracyByVariableShuffling[clFunc_?ClassifierQ, testData_?ClassifierDataQ, variableNames : (Automatic | {_String..}), opts : OptionsPattern[] ] :=
    Block[{ baseAccuracy, tmat, shuffledTestSets, accuraciesOfShuffledTestSets, varNames, fscoreLabels, targetClasses },
      ...
      Which[
        ...
        targetClasses === None,
        baseAccuracy = EnsembleClassifierMeasurements[ clFunc, testData, "Accuracy"],

        targetClasses =!= None,
        baseAccuracy = EnsembleClassifierMeasurements[ clFunc, testData, "Precision", "ClassLabels" -> targetClasses];
        baseAccuracy = baseAccuracy /@ targetClasses;
      ];
      ...
      Which[
        ...
        targetClasses === None,
        accuraciesOfShuffledTestSets =
            EnsembleClassifierMeasurements[clFunc, #, "Accuracy"] & /@ shuffledTestSets,

        targetClasses =!= None,
        accuraciesOfShuffledTestSets =
            EnsembleClassifierMeasurements[clFunc, #, "Precision", "ClassLabels" -> targetClasses] & /@ shuffledTestSets;
        accuraciesOfShuffledTestSets = Map[ # /@ targetClasses &, accuraciesOfShuffledTestSets];
      ];
...
AccuracyByVariableShuffling[___] :=
    (Message[AccuracyByVariableShuffling::noargs]; $Failed);
